...
@Component({
  ...
  imports: [NgIcon, ReactiveFormsModule, FormsModule],
...
export class CalorieCalculator {
  ...
  customForm: FormGroup = this.fb.group({
    customValue: [null, Validators.required]
  });
  ...
  setDailyCalories() {
    ...
    if(calories === null) {
      this.notificationService.showWarning('Pass a valid calorie value to set daily calorie goal');
      return;
    }
  ...
  validateCalories = computed(() => {
    const val = this.selectedCalorieValue();
  ...
  resetResult() {
    this.result.set(null);
    this.selectedCalorieValue.set(null);
  }
